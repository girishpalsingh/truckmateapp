...
-- Add to trailers
-- Note: trailers table might not exist if 20260122000004 was not applied. 
-- We assume it exists based on plan, but we'll use IF EXISTS just in case or create it if missing in a separate step? 
-- The user said "trailers table is seemingly missing" in my research, but I found the file defining it. 
-- If the user didn't run that migration, this will fail. 
-- START CHECK: We will attempt to create trailers if it doesn't exist, copying from previous definition slightly simplified for safety
...
-- Drivers can update loads assigned to them (e.g. status)
